...
void DobleSalto(char mat[LONG][LONG], int fila, int columna, char pieza, int turnoColor);
//-------------------------------------------------------------------------------------------------------------------
...
void LimpiarPantalla() {
    printf("Esperando para limpiar la pantalla...\n");
    sleep(1); // Esperar 2 segundos (puedes ajustar este tiempo)
...
void MostrarReglas() {
    int opcion;
    FILE *archivo = fopen("reglas.txt", "r");
    if (archivo == NULL) {
        printf("No se pudo abrir el archivo de reglas.\n");
        return;
    }

    char linea[256];
    printf("Reglas de las Damas:\n");
...
void Jugar(char mat[LONG][LONG], int *turnoColor) {
    ...
    while (!juegoTerminado) {
        ...
        juegoTerminado = VerificarFinJuego(mat, turnoColor);
...
void IngresarNombres() {
    ...
    if (VerificarNombre(nombreNegro)) {
        ...
        char confirmacion;
        printf("El nombre %s ya esta en uso. Es este tu nombre de usuario? (s/n): ", nombreNegro);
...
int VerificarFinJuego(char mat[LONG][LONG], int *turnoColor) {
    ...
    if (resultadoJuego != 0) {
        ...
        do {
            printf("\nJuego Finalizado\n");
            printf("Ingrese 1 para salir del juego\n");
            printf("Selecciona una opcion: ");
            scanf("%d", &opcion);
            if (opcion == 1) {
                printf("Saliendo del juego...\n");
...
void MovimientoR(char mat[LONG][LONG], int *turnoColor) {
    ...
    if (vectorCaptura[0] == 1) {
        ...
        if (mat[capturarY][capturarXP] == 'n' || mat[capturarY][capturarXP] == 'N') { // Captura de piezas negras o reinas negras
            ...
            DobleSalto(mat, filaN, columnaN, 'R', *turnoColor); // Llamar a DobleSalto si es posible
        }
    } else {
        do {
            ...
            if (mat[fila][columna] != 'R' && mat[fila][columna] != 'r') {
                ...
                // Lógica de movimiento para pieza normal que realiza una captura
                    ...
                    // Verificar posibilidad de doble salto
                    DobleSalto(mat, filaN, columnaN, 'R', *turnoColor); // Llamar a DobleSalto
                }
            ...
            // Si la pieza llega a la primera fila (índice 0), convertirla en reina
            if (mat[filaN][columnaN] == 'R' && filaN == 0) {
                mat[filaN][columnaN] = 'r';  // Convertir a reina
...
void MovimientoN(char mat[LONG][LONG], int *turnoColor) {
    ...
    if (vectorCaptura[0] == 1) {
        ...
        if (mat[capturarY][capturarXP] == 'R' || mat[capturarY][capturarXP] == 'r') { // Captura de piezas rojas o reinas rojas
            ...
            DobleSalto(mat, filaN, columnaN, mat[filaN][columnaN], *turnoColor); // Llamar a DobleSalto
        }
    } else {
        do {
            printf("Ingrese las coordenadas de la pieza que desea mover (primero fila, luego columna):\n");
...
void VerificarCapturaRojas(char mat[LONG][LONG], int vectorCaptura[3]) {
    int vectorAux[100][3];  // Para almacenar múltiples opciones de captura
    int fila, columna;
    int numCapturas = 0;

    for (fila = 0; fila < LONG; fila++) {
        ...
        for (columna = 0; columna < LONG; columna++) {
            if (mat[fila][columna] == 'N') {  // Si encontramos una pieza negra normal
                ...
                if (fila + 2 < LONG && columna - 2 >= 0 &&
                    ...
                    mat[fila + 2][columna - 2] == ' ') {
                    vectorAux[numCapturas][0] = 1;
                    vectorAux[numCapturas][1] = fila + 1;  // Guardamos la posición de la pieza que captura (en formato 1-indexed)
                    vectorAux[numCapturas][2] = columna + 1;
                    numCapturas++;
                }
                ...
                // Comprobar si puede capturar en todas las direcciones
                if ((fila - 2 >= 0 && columna - 2 >= 0 &&
                     (mat[fila - 1][columna - 1] == 'R' || mat[fila - 1][columna - 1] == 'r') &&
                     mat[fila - 2][columna - 2] == ' ') ||
...
//-------------------------------------------------------------------------------------------------------------------
void DobleSalto(char mat[LONG][LONG], int fila, int columna, char pieza, int turnoColor) {
    int filaN, columnaN, deltaX, deltaY, capturarXP, capturarY;
    int vectorCaptura[3] = {0, 0, 0};
    VerificarCaptura(mat, vectorCaptura, turnoColor);  // Esta función verifica las capturas posibles

    while (vectorCaptura[0] == 1) {
        printf("Pieza %c (%d, %d) puede realizar un segundo salto. \n", pieza, fila + 1, columna + 1);
        do {
            printf("Donde desea mover la pieza para el segundo salto? (primero fila, luego columna):\n");
            scanf("%d %d", &filaN, &columnaN);
            filaN--; columnaN--;  // Ajustamos a índices de 0
            deltaX = columnaN - columna;
            deltaY = filaN - fila;
                 ...
                 abs(deltaX) != 2 || abs(deltaY) != 2 || mat[filaN][columnaN] != ' ' ||
                 (mat[fila + deltaY / 2][columna + deltaX / 2] != (turnoColor == 0 ? 'n' : 'r') &&
                  mat[fila + deltaY / 2][columna + deltaX / 2] != (turnoColor == 0 ? 'N' : 'R'))); // Captura: la pieza enemiga debe estar en el medio

        // Realizar la captura
        capturarXP = columna + deltaX / 2;
        capturarY = fila + deltaY / 2;
        if (mat[capturarY][capturarXP] == (turnoColor == 0 ? 'n' : 'r') || mat[capturarY][capturarXP] == (turnoColor == 0 ? 'N' : 'R')) {
            mat[capturarY][capturarXP] = ' ';  // Eliminar la pieza capturada
            mat[fila][columna] = ' ';
            mat[filaN][columnaN] = pieza;
        }

        fila = filaN;
        columna = columnaN;
        VerificarCaptura(mat, vectorCaptura, turnoColor);
    }
}

int VerificarCaptura(char mat[LONG][LONG], int vectorCaptura[3], int turnoColor) {
    char piezaEnemiga = (turnoColor == 0) ? 'n' : 'r';
    char reinaEnemiga = (turnoColor == 0) ? 'N' : 'R';

    for (int i = 0; i < LONG; i++) {
        for (int j = 0; j < LONG; j++) {
            if (mat[i][j] == (turnoColor == 0 ? 'R' : 'N') || mat[i][j] == (turnoColor == 0 ? 'r' : 'n')) {
                // Verificar las cuatro direcciones para una captura posible
                int direcciones[4][2] = {{-2, -2}, {-2, 2}, {2, -2}, {2, 2}};

                for (int k = 0; k < 4; k++) {
                    int nuevaFila = i + direcciones[k][0];
                    int nuevaColumna = j + direcciones[k][1];
                    int midFila = i + direcciones[k][0] / 2;
                    int midColumna = j + direcciones[k][1] / 2;

                    if (nuevaFila >= 0 && nuevaFila < LONG && nuevaColumna >= 0 && nuevaColumna < LONG &&
                        mat[nuevaFila][nuevaColumna] == ' ' &&
                        (mat[midFila][midColumna] == piezaEnemiga || mat[midFila][midColumna] == reinaEnemiga)) {
                        vectorCaptura[0] = 1;
                        vectorCaptura[1] = i + 1;
                        vectorCaptura[2] = j + 1;
                        return 1; // Captura posible encontrada
                    }
                }
            }
        }
    ...
    vectorCaptura[0] = 0; // No hay capturas posibles
    return 0;
}
