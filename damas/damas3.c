...
// Función para guardar el estado de la partida en un archivo numerado
void GuardarPartida(char mat[LONG][LONG], int turnoColor) {
    int i = 1;
    char filename[20];
    FILE *archivo;

    // Buscar un archivo único para guardar la partida
    while (1) {
        snprintf(filename, sizeof(filename), "partida_guardada_%d.txt", i);  // Incluye el número de la partida
        archivo = fopen(filename, "r");
        if (!archivo) break;  // Si el archivo no existe, podemos crear uno nuevo
        fclose(archivo);
        i++;  // Incrementamos el número para buscar otro archivo
    }
    ...
    if (opcion == 2) {
        ...
        printf("Opción no válida. Continuando con el juego...\n");
    }
}


// Función que carga el estado de la partida desde un archivo
int CargarPartida(char mat[LONG][LONG], int *turnoColor) {
    int partida;
    printf("Seleccione el número de la partida a cargar:\n");

    // Mostrar las partidas guardadas
    for (int i = 1; i <= 10; i++) {
        char filename[20];
        snprintf(filename, sizeof(filename), "partida_guardada_%d.txt", i);  // Nombre del archivo con el número de partida
        FILE *archivo = fopen(filename, "r");
        if (archivo) {
            printf("%d. %s\n", i, filename);  // Mostrar archivo si existe
            fclose(archivo);  // Cerrar el archivo después de comprobar su existencia
        }
    }

    printf("Ingrese el número de la partida que desea cargar (0 para cancelar): ");
    scanf("%d", &partida);
    ...
    char filename[20];
    snprintf(filename, sizeof(filename), "partida_guardada_%d.txt", partida);  // Nombre del archivo con el número de partida
