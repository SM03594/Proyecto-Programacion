...
int VerificarFinJuego(char mat[LONG][LONG], int finJuego, int *Finjuego, int turnoColor);
    ...
    while (1) {
        ...
        switch (opcion) {
            case 1:
                ...
                IngresarNombres();
                CrearMatriz(tablero);  // Inicializar y rellenar el tablero con piezas
...
int VerificarFinJuego(char mat[LONG][LONG], int finJuego, int *Finjuego, int turnoColor) {
    ...
    // Contar las piezas de cada jugador
    for (i = 0; i < LONG; i++) {
        for (j = 0; j < LONG; j++) {
            if (mat[i][j] == 'N' || mat[i][j] == 'n') {
                contarNegras++;  // Contamos las piezas negras
            } else if (mat[i][j] == 'R' || mat[i][j] == 'r') {
                contarRojas++;  // Contamos las piezas rojas
            }
        }
    }
    ...
    if (contarRojas == 0) {
        ...
        *Finjuego = 1;  // Rojas ganan
        ...
        if (PuedeMover(mat, turnoColor) == 0) {
            if (turnoColor == 1) {
                *Finjuego = 2;  // El jugador negro gana porque las rojas no pueden mover
            } else if (turnoColor == 0) {
                *Finjuego = 1;  // El jugador rojo gana porque las negras no pueden mover
            }
        }
    }

    // Si el juego ha terminado, mostrar el resultado
    if (*Finjuego == 1) {
        printf("El jugador Rojo ha ganado!\n");

        // Pedir el nombre del jugador rojo (no debe ser "Rojo")
        char nombre[50];
        printf("Ingrese el nombre del jugador rojo: ");
        scanf("%s", nombre);  // Ingresar el nombre del ganador real
        if (strcmp(nombre, "Rojo") != 0) {
            ActualizarPuntajes(nombre, 1);  // Sumar un punto al jugador rojo (nombre real)
        }
    } else if (*Finjuego == 2) {
        printf("El jugador Negro ha ganado!\n");

        // Pedir el nombre del jugador negro (no debe ser "Negro")
        char nombre[50];
        printf("Ingrese el nombre del jugador negro: ");
        scanf("%s", nombre);  // Ingresar el nombre del ganador real
        if (strcmp(nombre, "Negro") != 0) {
            ActualizarPuntajes(nombre, 1);  // Sumar un punto al jugador negro (nombre real)
        }
    }
    ...
    do {
        ...
        if (opcion == 2) {
            ...
            return 0;  // Regresar al menú (se puede hacer con una bandera o modificando el flujo del programa)
        } else {
            printf("Opción no válida. Intente nuevamente.\n");
        }
    } while (opcion != 1 && opcion != 2);  // Repetir hasta que se seleccione una opción válida

    return finJuego;  // Esto debería devolver el resultado final
}
